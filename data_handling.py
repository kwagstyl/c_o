...
def dilate_values_inside_cortex(angles_image,angles_mask, cortex_mask):
    ...
    old_image=np.array([1])
    angles_mask=np.array(angles_mask)
    while np.sum(new_image)!=np.sum(old_image):
        old_image=new_image.copy()
        
        new_image[angles_mask==0]+=cv2.dilate(old_image,kernel, iterations = 1)[angles_mask==0]
        angles_mask=cv2.dilate(angles_mask,kernel,iterations=1)
